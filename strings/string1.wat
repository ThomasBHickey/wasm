...
(module
 ...
 (func $i32.hexprint (param $N i32)
  ...
  (func $wam2wat (param $strPtr i32)(result i32)
	...
	(call $printwsp (global.get $gtree))
  ...
  (func $pushExpr (param $state i32)
	...
	(local.set $tree (call $map.get (local.get $state)(global.get $gtree)))
	(call $i32list.push (local.get $tree)(local.get $newExpr))
	(call $printwlf (local.get $state))
  )
  ...
  (func $wamTokenize (param $strPtr i32)(result i32)
	...
	(call $map.set (local.get $state)(global.get $gtree)(local.get $top))
	...
	(call $map.get (local.get $state) (global.get $gtree))
  )
