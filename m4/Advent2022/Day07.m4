...
(func $printFile (param $file i32)(param $level i32)
  ...
  (call $str.print(global.get $strFile))(call $byte.print _CHAR(`:'))
  (call $str.print(call $map.get(local.get $file)(global.get $strName)))
  (call $i32.print(call $map.get(local.get $file)(global.get $strLength)))
  (call $printlf)
)
(func $printElem (param $elem i32)(param $level i32)
  ...
  (call $printIndent (local.get $level))
  _incrLocal($level)
  (call $str.print(global.get $strName))(call $byte.print _CHAR(`:'))
  (call $str.print (call $map.get (local.get $elem)(global.get $strName)))(call $printlf)
  ...
  (call $str.print(global.get $strType))
    (call $byte.print _CHAR(`:'))
    (call $printwlf (local.get $elemType)) ;; should be 'dir'!
	  ...
	  (then
	    ...
	    (if (call $str.compare
		  ...
		  (then
			...
			;;(call $printwlf (call $childToStr (local.get $child)))
			;;(call $childrenToStr (local.get $child))
			(call $printElem(local.get $child) _0)
...
(func $dirCommand (param $root i32)(param $line i32)
  ...
  (call $printElem(local.get $newDir) _0)
...
(func $Day07a (export "_Day07a")
  ...
  _testString(`galldone',`Finished input here is file system:')
