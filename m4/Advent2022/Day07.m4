...
(func $printElem (param $dir i32)(param $level i32)
  (local $dirType i32)(local $children i32)(local $numChildren i32)(local $childNum i32)
  (call $printIndent (local.get $level))
  _incrLocal($level)
  (call $print(global.get $strName))(call $printlf)
  (local.set $dirType (call $map.get (local.get $dir)(global.get $strType)))
  (if (call $str.compare (local.get $dirType)(global.get $strFile))
	(then
	  (call $printFile (local.get $dir)(local.get $level))
	  (return)))
  ;; if not a 'file' then it is a directory
  (call $printwlf (local.get $dirType)) ;; should be 'dir'!
  (local.set $children (call $map.get (local.get $dir)(global.get $strChildren)))
...
(func $childToStr (param $child i32)(result i32)
  (local $strPtr i32)(local $childType i32)

  (local.set $strPtr (call $str.mk))
  (call $str.catStr (local.get $strPtr)(call $map.get (local.get $child)(global.get $strName)))
  (call $str.catByte (local.get $strPtr) _SP)
  (local.set $childType (call $map.get (local.get $child)(global.get $strType)))
  (if (call $str.compare(local.get $childType)(global.get $strDir))
	(call $str.catStr (local.get $strPtr) (call $fileSystemToStr (local.get $child))))
  (call $str.catByte (local.get $strPtr) _SP)
  (local.get $strPtr)
)
...
;;	      (call $map.get (local.get $child)(global.get $strName)))
;;		(call $str.catByte (local.get $strPtr) _SP)
		(call $str.catStr (local.get $strPtr)(call $childToStr(local.get $child)))
		_incrLocal($childNum)
		(br $childLoop))))
  (call $str.catByte (local.get $strPtr) _CHAR(`]'))
  (local.get $strPtr)
