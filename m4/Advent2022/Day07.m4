...
(func $cdCommand (param $root i32)(param $line i32)(result i32)
  (local $cdName i32)(local $children i32)(local $child i32)(local $numChildren i32)(local $childNum i32)
...
(func $lsCommand (param $root i32)(param $line i32)
  ...
  (loop $lineLoop
	...
	(else
		...
		(call $addFile (local.get $root)(local.get $line))
	))
	(if (i32.ne (local.get $lineTerm) _maxNeg)
	  (br $lineLoop))
  )
)	
(func $parseLine (param $root i32)(param $line i32)(result i32)
  _testString(`ginparseline',`in parseLine')
  (if (call $str.startsAt (local.get $line)(global.get $strCd) _0)
	(then (return (call $cdCommand(local.get $root)(local.get $line)))))
