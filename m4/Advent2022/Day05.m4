...
(func $getFromEnd (param $stack i32)(param $num i32)
  (local $counter i32)(local $tempList i32)
  (local.set $tempList (call $i32list.mk))
  (local.set $counter _0)
  (loop $crateLoop
    (if (i32.lt_s (local.get $counter)(local.get $num))
	  (then
		(call $i32list.push (local.get $tempList)(call $i32list.pop (local.get $stack)))
		_incrLocal($counter)
		(br $crateLoop)
	  )
	)
  )
  (call $i32list.reverse(local.get $tempList))
)
(func $doMovesB (param $stacks i32)(param $moves i32)
  (local $moveSize i32)(local $moveFrom i32)(local $moveTo i32)
  (local $move i32)(local $moveCounter i32)(local $crate i32)(local $movePos i32)
  (local.set $movePos _0)(local picked i32)
  ...
  (loop $moveLoop
	...
	(local.set $moveFrom (call $i32list.get@ (local.get $move) _1))
	  _decrLocal($moveFrom)
	(local.set $moveTo   (call $i32list.get@ (local.get $move) _2))
  	(local.set $picked (call $getFromEnd (i32list.get@ (local.get $stacks)(local.get $moveFrom)(local.get $moveSize))))
	(call $i32list.cat (i32list.get@ (local.get $stacks)(local.get $moveTo))(local.get $picked))
	  _decrLocal($moveTo)
)
	(loop $doMove  :::!!! loop not needed now
	  _incrLocal($moveCounter)
	  (if (i32.le_s (local.get $moveCounter)(local.get $moveSize))
	    (then
		  _incrLocal(
