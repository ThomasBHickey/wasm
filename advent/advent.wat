...
(module
  ...
  ;; (func $row.score (param $row i32)(result i32)
	...
	;; (loop $colLoop
	  ...
	  ;; (local.set $score 
		;; (i32.add (local.get $score)(call $i32list.get@(local.get $colNum))))
	  ;; (local.set $colNum (i32.add (local.get $colNum)(i32.const 1)))
	  ;; (br $colLoop)
	;; )
  ;; )
  (func $row.score (param $row i32)(result i32)(i32.const 33))
  (func $chart.score (param $row i32)(result i32)(i32.const 77))
  ;; (func $chart.score (param $chart i32)(result i32)
	...
	;; (call $printwlf (local.get $numRows))
	;; (local.set $score (i32.const 0))
	;; (local.set $rowNum (i32.const 0))
	;; (loop $rowLoop
	  ;; (if (i32.ge_u (local.get $rowNum)(local.get $numRows))(return (local.get $score)))
	  ;; (local.set $score (call $row.score (call $i32list.get@ (local.get $chart)(local.get $rowNum))))
	  ;; (br $rowLoop)
	;; )
	;; ;;(local.get $score)
  ;; )
