...
(module
  ...
  (func $chart.addSeg (param $chart i32)(param $lineSeg i32)
	...
	(if (i32.eq (local.get $x1)(local.get $x2))
	  (then  ;; vertical line
		...
		(call $printwsp(local.get $y1))(call $printwlf (local.get $y2))
	  ...
	  (else ;; horizontal line?
	   (if (i32.eq (local.get $y1)(local.get $y2))
		(then
		  ...
		  (call $printwsp (global.get $gF))(call $printwsp(local.get $x1))(call $printwlf (local.get $x2))
		  (local.set $x (local.get $x1)
		  (local.set $y (local.get $y1))
		  (call $printwsp (local.get $x1))
		  (call $printwsp (local.get $x1))
		  (call $printwsp (global.get $gC))(call $printwsp(local.get $x))(call $printwlf(local.get $y))
		  (call $printwsp (global.get $gC))(call $printwsp(local.get $x))(call $printwlf(local.get $y))
		  (loop $xloop
			(call $chart.addPoint (local.get $chart)(call $point.mk (local.get $x)(local.get $y)))
  ...
  (func $day5 (export "_day5")
	...
	(loop $lineLoop
	  ...
	  (call $printwsp (local.get $lineNum))(call $printwlf (local.get $lineSeg))
