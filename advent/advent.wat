...
(module
  ...
  (func $point.mk (param $x i32)(param $y i32)(result i32)
	...
	;; (call $i32list.push (local.get $x))
	;; (call $i32list.push (local.get $y))
	(local.get $pt)
  )
  ...
  (func $chart.mk (param $numRows i32)(param $numCols i32)(result i32)
    (local $chart i32)(local $blankRow i32)(local $row i32)
	(local $rows i32)(local $rowNum i32)(local $tempRow i32)
	(local $colNum i32)
	(local.set $blankRow (call $str.mk))
	(local.set $colNum (i32.const 0))
	(loop $cLoop
  ...
  (func $chart.addPoint (param $chart i32)(param $point i32)
	...
	(local.set $row (call $i32list.get@ (local.get $chart)(call $point.getY (local.get $point))))
	(call $printwlf (global.get $gC))
	(call $printwlf (local.get $row))
	(call $printwlf (global.get $gD))
	;;(call $i32list.set@ (local.get $row)(call $point.getX (local.get $point)))
  )
