...
(module
  ...
  (func $point.mk (param $txtRep i32)(result i32)
	(local $pt i32)(local $xyReps i32)
  ...
  (func $lineSeg.mk (param $txtRep i32)(result i32)
	...
	(local $xpt i32)(local $ypt i32)(local $xptTxt i32)(local $yptTxt i32)
	;;(call $printwlf (local.get $txtRep))
	(local.set $arrowStr (call $str.mkdata (global.get $gArrow)))
	(local.set $arrowPos (call $str.find (local.get $txtRep)(local.get $arrowStr)))
	;;(call $printwlf (local.get $arrowPos))
	(local.set $xptTxt (call $str.mkslice (local.get $txtRep)(i32.const 0)(local.get $arrowPos)))
	(call $printwlf (local.get $xptTxt))
	(local.set $xpt (call $point.mk (local.get $xptTxt)))
	;;(call $printwlf (local.get $xpt))
	(local.set $yptTxt 
	  (call $str.mkslice
	    (local.get $txtRep)
	...
	(call $printwlf (local.get $yptTxt))
	(local.set $ypt (call $point.mk (local.get $yptTxt)))
	(call $printwlf (local.get $ypt))
	(local.set $seg (call $i32list.mk))
	(call $i32list.push (local.get $seg)(local.get $xpt))
	(call $i32list.push (local.get $seg)(local.get $ypt))
	(call $printwlf (local.get $seg))
	(local.get $seg)
  )
  (func $chart.mk (param $maxRow i32)(param $maxCol i32)(result i32)
    (local $chart i32)
	(local $rows i32)(local $rowNum i32)
	(local $colNum i32)
	(local.set $rowNum (i32.const 0))
	(loop $rloop
	  (local.set $colNum (i32.const 0))
	  (loop $cloop
		(local.set $colNum (i32.add (local.get $colNum)(i32.const 1)))
		(if (i32.lt_u (local.get $colNum)(local.get $maxCol))
		  (br $cloop)))
	  (local.set $rowNum (i32.add (local.get $rowNum)(i32.const 1)))
	  (if (i32.lt_u (local.get $rowNum)(local.get $maxRow))
	    (br $rloop)))	
	(local.get $chart)
  )
  (func $day5 (export "_day5")
	...
	(local.set $chart (call $chart.mk (i32.const 10)(i32.const 10)))
	(local.set $chart (call $chart.mk (i32.const 10)(i32.const 10)))
