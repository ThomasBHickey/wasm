...
(module
  ...
  (func $strList2intList (param $strList i32)(result i32)
	...
	(loop $numLoop
	  ...
	  (call $printwlf (local.get $textNum))
  ...
  (func $board.mk (param $lines i32)(param $linePos i32)(result i32)
    (local $board i32)
  ...
  (func $day4 (export "_day4")
	...
	(local $ranNums i32)(local $data i32)
	(local.set $boardSize (i32.const 5))
	(local.set $data (call $readFileSlow))
	(local.set $firstLine (call $i32list.get@ (local.get $data)(i32.const 0)))
	(local.set $numLines (call $i32list.getCurLen (local.get $data)))
	...
	(loop $boardLoop
	  (local.set $linePos (i32.add (local.get $linePos)(i32.const 5)))
