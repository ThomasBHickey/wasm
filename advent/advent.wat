...
(module
  ...
  (func $row.score (param $row i32)(result i32)
	...
	(loop $colLoop
	  ...
	  ;;(call $printwlf (call $i32list.get@(local.get $row)(local.get $colNum)))
	  (if (i32.gt_u(call $i32list.get@(local.get $row)(local.get $colNum)) (i32.const 1))
  ...
  (func $chart.score (param $chart i32)(result i32)
	...
	(loop $rowLoop
	  ...
	  (local.set $score (call $row.score (local.get $row)))
	  (local.set $rowNum (i32.add (local.get $rowNum)(i32.const 1)))
	  (br $rowLoop)
	)
	(local.get $score)
  ...
  (func $chart.print (param $chart i32)
	...
	(loop $rLoop
	  (if (i32.lt_u (local.get $rowNum)(local.get $numRows))
		(then
		  (call $printwlf (call $chart.getRow (local.get $chart)(local.get $rowNum)))
		  (local.set $rowNum (i32.add (local.get $rowNum)(i32.const 1)))
		  (if (i32.lt_u (local.get $rowNum)(local.get $numRows))
			(br $rLoop)))))
  )
  ...
  (func $chart.addSeg (param $chart i32)(param $lineSeg i32)
	...
	(call $printwsp (global.get $gSeg))(call $printwlf (local.get $lineSeg))
	(if (i32.eq (local.get $x1)(local.get $x2))
	  (then  ;; vertical line
		(if (i32.gt_s (local.get $y1)(local.get $y2))
		  (then  ;; lower to higher value
			(local.set $temp (local.get $y1))
			(local.set $y1 (local.get $y2))
			(local.set $y2 (local.get $temp))))
		...
		(loop $yloop
		  ...
		  (if (i32.le_s (local.get $y)(local.get $y2))
		    (br $yloop))
		)
		;;(call $chart.print (local.get $chart))
	  )
	  (else ;; horizontal line?
	   (if (i32.eq (local.get $y1)(local.get $y2))
		(then
		  (if (i32.gt_s (local.get $x1)(local.get $x2))
		  (then ;; swap the x's if necessary
			(local.set $temp (local.get $x1))
			(local.set $x1 (local.get $x2))
			(local.set $x2 (local.get $temp))
			))
		  (call $printwsp (local.get $x1))(call $printwlf (local.get $x2))
		  (local.set $x (local.get $x1)
		  (local.set $y (local.get $y1))
		  (loop $xloop
		    (call $printwsp (local.get $x))(call $printwlf (local.get $y))
			(call $chart.addPoint (local.get $chart)(call $point.mk (local.get $x)(local.get $y)))
			(local.set $x (i32.add (local.get $x)(i32.const 1)))
			(if (i32.le_s (local.get $x)(local.get $x2))
			  (br $xloop)))
		  )
		;;(call $chart.print (local.get $chart))
		)
  ...
  (func $day5 (export "_day5")
	...
	(local $line i32)(local $lineNum i32)(local $lineSeg i32)
	(local $chartNumRows i32)(local $chartNumCols i32)
	(local $numLines i32)
	(local.set $chartNumRows (i32.const 10))
	(local.set $chartNumCols (local.get $chartNumRows))  ;; square
	...
	(call $printwlf (call $chart.score (local.get $chart)))
