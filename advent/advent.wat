...
(module
  ...
  (func $row.score (param $row i32)(result i32)
    (local $score i32)(local $colVal i32)(local $colScore i32)
	...
	(loop $colLoop
	  ...
	  (local.set $colScore (i32.const 0))
	  (local.set $colVal (call $i32list.get@(local.get $row)(local.get $colNum)))
	  (if (local.get $colVal)
		(local.set $colScore (i32.sub (local.get $colVal)(i32.const 1))))
      (local.set $score (i32.add (local.get $score)(local.get $colScore)))
	  (local.set $colNum (i32.add (local.get $colNum)(i32.const 1)))
	  (br $colLoop)
	)
	(local.get $score)
